...
class DecodeRouter():
    def __init__(
        ...
        if policy not in ["rr", "lfl", "hfl", "hl", "ll", "ln", "tile_aware", "fadr"]:
    ...
    def route(self, request_data: Optional[dict] = None):
        ...
        if self.policy == "rr":
        ...
        elif self.policy =="lfl":
            decode_server = self._lowest_frequency_latency_decode_server()
        elif self.policy == "hfl":
            decode_server = self._highest_frequency_load_decode_server()
        elif self.policy == 'hl':
            decode_server = self._highest_load_decode_server()
        elif self.policy == 'll':
            decode_server = self._lowest_load_decode_server()
        elif self.policy == 'ln':
            ...
            # decode_server = self._fadr(request_data)
    ...
    def _lowest_frequency_latency_decode_server(self):
        sorted_servers = sorted(self.servers_info.items(), key=lambda x: (x[1][0], x[1][1]))
        return sorted_servers[0][0]

    def _lowest_load_decode_server(self):
        servers_info  = self.get_decode_server_info()
        sorted_servers = sorted(
            servers_info,
            key=lambda x: x[1]["load"],  # Sort by load
        )
        return sorted_servers[0][0]

    def get_decode_server_info(self):
        return [info for info in self.servers_info.items() if info[1]["is_prefill"] is False]
    ...
    def _highest_load_decode_server(self):
        slo = next(iter(self.servers_info.values()))["slo"]  # Get SLO from the first server
        sorted_servers = sorted(
            (item for item in self.servers_info.items() if item[1][-1] <= slo),
            key=lambda x: -x[1]["load"],  # Sort by load
        )
        if not sorted_servers:
            sorted_servers = sorted(self.servers_info.items(), key=lambda x: -x[1][4])
        return sorted_servers[0][0]

    def _highest_frequency_load_decode_server(self):

        slo = next(iter(self.servers_info.values()))["slo"]  # Get SLO from the first server
        sorted_servers = sorted(
            (item for item in self.servers_info.items() if item[1]["last_itl"] <= slo*0.8),
            key=lambda x: (-x[1]["freq"], -self.lb.n_req[x[0]]),  # Sort by frequency and then by number of requests
        )
        if not sorted_servers:
            sorted_servers = sorted(
                (item for item in self.servers_info.items()),
                key=lambda x: x[1]["last_itl"],  # Sort by frequency and then by number of requests
            )
            # return self._tile_aware_sche()
        return sorted_servers[0][0]

    def _fadr_jiahuan(self, request_data: dict):
        # under 64 reqs: balance
        servers_info  = [info for info in self.servers_info.items() if info[1]["is_prefill"] is False]
        reqs = [self.lb.n_req[server] for server, _ in servers_info]
        if max(reqs) < 64:
            sorted_servers = sorted(
        ...
        # else: energy-aware scheduling

        freq_list = [1005, 1095, 1200, 1305, 1410]
        slo = servers_info[0][1]["slo"]

        server_freq_slo_allowed = dict()  # servers that can satisfy the SLO

        tokens = self.tokenizer.encode(request_data["text"])
        for server, info in servers_info:
            bs = self.lb.n_req[server]
            load = info["load"]
            for freq_ in freq_list:
                itl_ = self.latency_lookup_table.lookup(bs=bs+1, tokens=load + len(tokens), freq=freq_)
                if itl_ is not None and itl_ <= slo-3:
                    server_freq_slo_allowed[server] = (freq_, itl_)
                    logger.debug(f"Server {server} with frequency {freq_} satisfies the SLO {slo} with itl {itl_}")
                    break  # found a frequency that satisfies the SLO, move to the next server

                # if freq_ == freq_list[-1]:
                #     server_freq_slo_allowed[server] = (freq_, itl_)
                #     logger.warning("No server satisfies the SLO, use highest frequency")


        if len(server_freq_slo_allowed) == 0:
            # for server, info in servers_info:
            #     bs = self.lb.n_req[server]
            #     load = info["load"]
            #     itl_ = self.latency_lookup_table.lookup(bs=bs+1, tokens=load + len(tokens), freq=freq_list[-1])
            #     server_freq_slo_allowed[server] = (freq_list[-1], itl_)
            #     logger.warning("No server satisfies the SLO, use highest frequency")

            sorted_servers = sorted(
                self.servers_info.items(),
                key=lambda x: x[1]["last_itl"]
            )
            return sorted_servers[0][0]

        energy_diff = dict()
        for server, (freq_, _) in server_freq_slo_allowed.items():
            bs = self.lb.n_req[server]
            load = self.servers_info[server]["load"]
            freq = self.servers_info[server]["freq"]
            # TODO: problem: freq may > freq_ ? caused by freq governor delay
            if freq > freq_:
                logger.warning(f"freq {freq} > freq_ {freq_} for server {server}")
            energy = self.energy_lookup_table.lookup(bs=bs, tokens=load, freq=freq)  # current
            energy_ = self.energy_lookup_table.lookup(bs=bs+1, tokens=load+len(tokens), freq=freq_) # expected
            energy_diff[server] = energy_ - energy

        sorted_servers = sorted(
            energy_diff.items(),
            key=lambda x: x[1]  # sort by energy difference
        )
        # print(f"Sorted servers by energy difference: {sorted_servers, self.lb.n_req}")
        return sorted_servers[0][0]
